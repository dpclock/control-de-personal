            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  const SizedBox(width: 8),
                  GestureDetector(
                    onTap: () => _mostrarDialogoConfirmacionBorrado(nombre, 'trabajador'),
                    child: Container(
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  const SizedBox(width: 8),
                  GestureDetector(
                    onTap: () => _mostrarDialogoConfirmacionBorrado(obra, 'proyecto'),
                    child: Container(
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(
                        color: const Color(0xFFEF4444).withOpacity(0.1),
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (nombreController.text.isNotEmpty && nombreController.text != nombreActual) {
                  try {
                    await _dataManager.actualizarTrabajador(nombreActual, nombreController.text.trim());
                    setState(() {}); // Actualizar la UI
                    Navigator.of(context).pop();
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (obraController.text.isNotEmpty && obraController.text != obraActual) {
                  try {
                    await _dataManager.actualizarObra(obraActual, obraController.text.trim());
                    setState(() {}); // Actualizar la UI
                    Navigator.of(context).pop();
...
class _TablaExcelPageState extends State<TablaExcelPage> with SingleTickerProviderStateMixin {
  ...
  List<int> _getFilasFiltradasYOrdenadas(int mesIndex) {
    ...
    // Si no hay filtros activos, mostrar todas las filas (0-99)
    if (_filtroNombre.isEmpty && _filtroObra.isEmpty && _ordenColumna.isEmpty) {
      filasConDatos = List.generate(100, (index) => index);
    } else {
      ...
      builder: (BuildContext context) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            String tipoElementos = items == _dataManager.trabajadores ? 'trabajadores' : 'obras';
                ...
                child: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        'Mostrando ${filteredItems.length} de ${items.length} $tipoElementos',
                        style: const TextStyle(
                          fontSize: 12,
                          color: Colors.grey,
                        ),
                    ...
                    Expanded(
                      child: filteredItems.isEmpty
                          ...
                          if (index == 0) {
                            ...
                            return ListTile(
                              leading: const Icon(Icons.space_bar, color: Colors.grey),
                              title: Text(
                                '-',
                                style: TextStyle(
                                  fontSize: fontSize,
